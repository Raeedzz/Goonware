 ...
 *
 * Links the user encounters — agent output, editor markdown, log lines —
 * open in their real browser; clicking one is "I want to look at this
 * in my real browser."
 *
...
export function useOpenUrlInBrowser() {
  useEffect(() => {
    ...
    window.addEventListener("goonware:open-url", onOpen);
    return () => window.removeEventListener("goonware:open-url", onOpen);
  }, []);
}
