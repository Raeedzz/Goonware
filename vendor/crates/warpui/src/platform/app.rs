...
impl AppBackend {
    ...
    fn attach_embedded(
        self,
        parent_nswindow: *mut std::ffi::c_void,
        init_fn: impl FnOnce(&mut AppContext, LocalBoxFuture<'static, crate::App>) + 'static,
    ) {
        match self {
            AppBackend::CurrentPlatform(inner) => inner.attach_embedded(parent_nswindow, init_fn),
            AppBackend::Headless(_) => {
                panic!("attach_embedded is only supported on the native platform backend")
            }
        }
    }
}
...
impl AppBuilder {
    ...
    /// Attaches the application to a host-owned run loop instead of
    /// starting warpui's own (see [`super::current::App::attach_embedded`]).
    /// Used to embed a warpui surface inside another application's window
    /// (e.g. a Tauri `NSWindow`), where the host already runs `NSApp`.
    pub fn attach_embedded(
        self,
        parent_nswindow: *mut std::ffi::c_void,
        init_fn: impl FnOnce(&mut AppContext) + 'static,
    ) {
        self.inner
            .attach_embedded(parent_nswindow, |ctx, _| init_fn(ctx));
    }
}
