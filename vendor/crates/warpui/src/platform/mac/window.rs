...
impl Window {
    pub fn open(
        ...
        unsafe {
            ...
            // Record this window so an embedded host (Tauri) can reparent it
            // reliably — it can't depend on AppKit key-window state, which the
            // host owns. No-op cost for the standalone (non-embedded) path.
            super::app::note_created_window(native_window);
...
extern "C-unwind" fn warp_update_layer(this: &Object) {
    {
        // Diagnostic (embedded spike): this fires only when macOS asks the
        // layer to present — i.e. the window is actually on-screen. Repeated
        // hits ⇒ the embedded surface is genuinely displaying under the
        // host's run loop (a signal we can read without a screenshot).
        use std::sync::atomic::{AtomicU32, Ordering};
        static UPDATES: AtomicU32 = AtomicU32::new(0);
        let n = UPDATES.fetch_add(1, Ordering::Relaxed);
        if n < 5 || n % 120 == 0 {
            eprintln!("[warpui] warp_update_layer (present) #{n}");
        }
    }
