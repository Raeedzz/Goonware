...
impl AppContext {
    ...
    /// Inject a synthetic input event into `window_id` from outside the platform
    /// event loop, running it through the normal hit-testing / selection /
    /// scroll dispatch unchanged. Embedded hosts (e.g. Tauri) use this because
    /// the embedded child window has `ignoresMouseEvents: YES` — mouse events are
    /// captured in the host's own layer and forwarded here. Returns whether the
    /// event was handled. No-op (returns false) if `window_id` has no presenter.
    pub fn inject_window_event(&mut self, window_id: WindowId, event: Event) -> bool {
        let Some(presenter) = self.presenter(window_id) else {
            return false;
        };
        self.handle_window_event(event, window_id, presenter)
