...
impl AppCallbackDispatcher {
    ...
    /// Force the embedded app to repaint: mark every view invalid and flush
    /// effects, which fires each window's invalidation callback →
    /// `request_redraw()`. Used by host apps (e.g. Tauri) to drive a redraw
    /// from outside the normal event flow when new render state arrives on
    /// another thread. Must be called on the main thread.
    pub fn poke_redraw(&mut self) {
        self.ui_app.update(|ctx| ctx.invalidate_all_views());
    }

    /// Request an off-screen capture of `window_id`'s next presented frame.
    /// The callback receives the RGBA pixels. Used by embedded hosts to
    /// screenshot the surface for verification. Main-thread only.
    pub fn capture_window(
        &mut self,
        window_id: WindowId,
        callback: Box<dyn FnOnce(super::CapturedFrame) + Send + 'static>,
    ) {
        self.ui_app.update(|cx| {
            if let Some(window) = cx.windows().platform_window(window_id) {
                window.as_ctx().request_frame_capture(callback);
            }
        });
    }

    /// Inject a synthetic input event into `window_id`, running it through
    /// warpui's normal dispatch (hit-testing / selection / scroll). Embedded
    /// hosts forward input they captured in their own layer because the embedded
    /// child window has `ignoresMouseEvents: YES`. Main-thread only.
    pub fn inject_mouse_event(&mut self, window_id: WindowId, event: crate::event::Event) {
        self.ui_app.update(|ctx| {
            ctx.inject_window_event(window_id, event);
        });
    }
